...
pthread_mutex_t mutex; // Mutex declaration (Global)
...
void* compute_pi(void* rank) {
    ...
    // Protect the critical section with mutex
...
int main(int argc, char *argv[]) {
    ...
    sum = sum * (1.0 / 3.0) * (high - low) / N; // Final answer for Simpson's rule
    elapsed = finish - start; // Elpased time 
    double exact_pi = M_PI; 
    double compError = fabs(exact_pi - sum);
